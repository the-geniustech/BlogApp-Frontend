...
const EditPostForm = ({ currentPostDetails, id }) => {
  const [modalOpen, setModalOpen] = useState(false);
  const trigger = useRef(null);
  const [coverImage, setCoverImage] = useState(null);
  const [title, setTitle] = useState(currentPostDetails.title);
  const [summary, setSummary] = useState(currentPostDetails.summary);
  const [tags, setTags] = useState(currentPostDetails.tags);
  const [content, setContent] = useState(currentPostDetails.content);
  ...
  const handleChange = (field, value) => {
    ...
    switch (field) {
      case "coverImage":
        setCoverImage(value);
        setValue("coverImage", value || coverImage);
        break;
      case "title":
        setTitle(value);
      ...
      case "tags":
        setTags(value);
        setValue("tags", value.join(" ") || tags.join(" "));
        break;
      case "content":
        setContent(value);
  ...
  return (
    ...
    >
      ...
      <footer className="bottom-0 left-0 z-50 fixed flex items-center bg-inherit px-4 py-6 w-full h-24">
        <div className="mx-auto container">
          <Button size="large" isloading={isEditing}>
            Update Post
          </Button>
        </div>
      </footer>
